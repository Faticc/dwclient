-- Real OpenComputers modem test: Master + 3 Worker nodes.
-- Run this on the Master after starting cluster_worker.lua on all workers.
...
-- Replace these with the three modem addresses.
local WORKERS = {
...
local function decrypt_chunks_locally(ciphertext)
    ...
    for chunk_id = 1, chunk_count do
        ...
        -- Вычисляем IV: для первого чанка это ключ, для остальных — предыдущие 16 байт шифротекста
        local iv = start_index == 1 and key or ciphertext:sub(start_index - 16, start_index - 1)
        local chunk = ciphertext:sub(start_index, end_index)
        
        -- Локальная расшифровка чанка
...
-- Замеряем только время расшифровки
local start = os.clock()
