--[[
Pure-Lua SHA-1. Only used for the Notchian "server hash" (server_id + shared_secret +
server_public_key_der), which is at most a few hundred bytes -- a plain one-shot
implementation is plenty, no need for a streaming/incremental API.
]]
...
local function sha1(message)
    ...
    -- Padding: 0x80, then zero bytes, until length % 64 == 56, then 8-byte big-endian
    -- bit length. bit_len can exceed 2^32 in theory but never will for our use case
    -- (crypto handshake payloads are a few hundred bytes), so the high 4 length bytes
