-- Установка и обновление dwclient одной командой, из OpenOS или DwOS:
--
--   wget -f https://raw.githubusercontent.com/Faticc/dwclient/main/install.lua /tmp/i.lua && /tmp/i.lua
--
-- Второй и следующие разы то же самое делает update.lua, который лежит
-- рядом с установленным клиентом: это ровно этот же файл.
--
--   --repo=владелец/репо   откуда качать (по умолчанию Faticc/dwclient)
--   --branch=ветка         ветка (main)
--   --dir=подкаталог       где в репозитории лежит сборка (dist)
--   --to=/путь             куда ставить (по умолчанию /home/dwclient)
...
-- оборванная загрузка рабочий клиент не портит. Что стоит и откуда
-- обновлялись, помнится в <каталог>/.dwclient.
--
-- Устройство и куски кода взяты из установщика DwOS (Faticc/dwos,
-- install.lua) -- тот же формат манифеста, та же проверка.
...
-- Долгие циклы обязаны уступать управление, иначе компьютер решит, что
-- программа зависла, и выключит его.
...
for _, entry in ipairs(manifest.files) do
  ...
  elseif FORCE or not have then
    ...
    -- Свой файл сверяется по хэшу, а не по записи в .dwclient: правку
    -- руками надо заметить и перекачать, а не молча счесть актуальной.
...
for _, entry in ipairs(todo) do
  ...
  print(n .. " Б")
  write_state(state)   -- после каждого файла: прерванная установка продолжится
end
